...
import { ImportNotFoundError, ImportValidationError, importRawServiceCsv } from '../../services/imports'
...
export default defineEventHandler(async (event) => {
  try {
    ...
    return await importRawServiceCsv({
...
function getUploadFile(parts: MultiPartData[]) {
  ...
  return {
    filename: file.filename ?? 'raw-service.csv',
